...
export function buildTags(record) {
  const f = CFG.airtable.fields, tags = [];
  const condition = stringField(record, f.conditionRanking);
  const make = stringField(record, f.make);
  const model = stringField(record, f.model);
  const finish = stringField(record, f.finish);
  const year = stringField(record, f.year);
  const shipping = stringField(record, f.shippingProfile);
  if (make) tags.push(`reverbsync-make:${make}`);
  if (model) tags.push(`reverbsync-model:${model}`);
  if (finish) tags.push(`reverbsync-finish:${finish}`);
  if (year) tags.push(`reverbsync-year:${year}`);
  if (condition) tags.push(`reverbsync-condition:${condition}`);
  if (shipping) tags.push(`reverbsync-shipping-profile:${shipping}`);
  if (condition) tags.push(`Condition:${condition}`);
  return tags.join(", ");
}
...
export async function updateAirtableRecord(recordId, fields) {
  if (CFG.dryRun) { logger("info", "DRY_RUN would update Airtable", { recordId, fields }); return { dryRun: true }; }
...
export async function shopifyRequest(path, options = {}) {
  const base = `https://${CFG.shopify.storeDomain}/admin/api/${CFG.shopify.apiVersion}`;
  const res = await fetch(`${base}${path}`, {
    ...options,
    headers: {"X-Shopify-Access-Token": CFG.shopify.token, "Content-Type": "application/json", ...(options.headers || {})}
  });
...
export function buildShopifyProductPayload(record) {
  ...
  const bodyHtml = textToShopifyHtml(stringField(record, f.description));
  const sku = stringField(record, f.sku);
  const price = numberField(record, f.price);
  const qty = numberField(record, f.qty);
  const productType = stringField(record, f.productCategory);
  const vendor = stringField(record, f.make) || "A Flash Flood of Gear";
  ...
  const product = { title, body_html: bodyHtml, vendor, product_type: productType, status: "draft", tags, variants: [variant] };
  if (images.length) product.images = images;
  return { product };
}
export function shopifyAdminProductUrl(productId) {
  return `https://admin.shopify.com/store/${CFG.shopify.storeDomain.replace(".myshopify.com", "")}/products/${productId}`;
}
...
export async function updateInventoryItemCost(inventoryItemId, cost) {
  ...
  if (CFG.dryRun) { logger("info", "DRY_RUN would update Shopify inventory item cost", { inventoryItemId, cost }); return { dryRun: true }; }
  return shopifyRequest(`/inventory_items/${inventoryItemId}.json`, { method: "PUT", body: JSON.stringify({ inventory_item: { id: inventoryItemId, cost: String(cost) } }) });
}
